...
function QuickView({ setShowQuick, id, products }) {
    ...
    return (
        <>
            <div className={"fixed w-full h-screen sm:h-auto top-0 left-0 flex justify-center items-start overflow-y-scroll z-50"}>
                <div className={`relative bg-white max-w-screen-xl my-20 mb-20 rounded-lg z-10`}>
                    <div className="flex flex-wrap mt-5">
                        <div className="px-5 mb-7 w-full md:w-7/12">
                            <div className="w-full mb-4 overflow-hidden rounded-lg h-auto">
                                {product && (
                                    <img loading="lazy" className={"w-full rounded-lg quickView_product_image_new width"} src={product?.image} alt="" />
                                )}
                            </div>
                        </div>
                ...
                <div onClick={() => setShowQuick(false)} className="w-full h-screen bg-gray-900 bg-opacity-60 fixed top-0 right-0 cursor-pointer" />
            </div>
